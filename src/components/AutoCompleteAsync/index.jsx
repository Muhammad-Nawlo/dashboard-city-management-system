...
export default function AutoCompleteAsync({
  ...
  const handleSelectChange = (newValue) => {
    ...
    if (onChange && multiple) {
      if (newValue) onChange(newValue[outputValue] || newValue);
      if (newValue) setCurrentValue(newValue[outputValue] || newValue);
    }
  ...
  useEffect(() => {
    ...
    if (value && isUpdating.current && !multiple) {
      setOptions([value]);
      setCurrentValue(value[outputValue] || value); // set nested value or value
    ...
    if (value && isUpdating.current && multiple) {
      let ids = value.map(val => val._id);
      setOptions([ids]);
      setCurrentValue(ids || value); // set nested value or value
      onChange(ids || value);
      isUpdating.current = false;
    }
  }, [value]);
